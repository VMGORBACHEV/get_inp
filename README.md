...
This Python script facilitates the generation of input files for high-throughput quantum chemistry (QC) calculations using software like [**ORCA**](https://www.faccts.de/orca/), [**Gaussian**](https://gaussian.com/), and [**MobCal-MPI**](https://github.com/HopkinsLaboratory/MobCal-MPI). It is particularly suited for automating large-scale computational workflows.

## Features

- Processes an ensemble of conformers (e.g., typically obtained from [**CREST**](https://github.com/crest-lab/crest)) alongside user-provided "base" input files specifying QC parameters.
- Generates a **"ready-to-submit"** set of input files, each with a unique name and, where applicable (e.g., Gaussian), a unique identifier for checkpoint files.
- Supports high-throughput benchmarking studies, such as testing multiple **DFT methods** or climbing the **"Jacob's ladder"** of computational accuracy.
